...
// formProps is the shared JSON Schema for form tool parameters (LO-09).
// Computed once at startup instead of on every tool call.
...
// The value schema is taken from formProps so enum constraints are preserved.
func BuildFieldTool(fieldName string) []map[string]interface{} {
	fieldProp, ok := formProps[fieldName]
	if !ok {
